    ...
    rtc.on('__new_peer', function(data) {
        ...
        if (undefined !== data && null !== data) {
            ...
            for (var i = 0; i < currentPlayers; i++) {
                var playerDisplayName = findDBPlayerNameByName(data[i]);
                console.log('rebuild player : ' + data[i] + ", " + playerDisplayName);
                players[i] = new Player(data[i], playerDisplayName, defaultInitChips, true, 0);
            }
        }
    });
